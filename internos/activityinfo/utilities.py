...
def get_indicators_v4(ai_db,form_id):
    ...
    indicator_all_info = client.get_database_indicators_v4(indicator_id)
    indicator_list=indicator_all_info['elements']
    category_indicators=[x for x in indicator_list if x['type']=='enumerated' and (x['relevanceCondition']=="" or x['relevanceCondition'] is None)]
    ...
    for sub_indicator in sub_indicators:
        ...
        if len(sub_indicator['relevanceCondition'])>0:
            ...
            master_indicator = [x for x in indicator_list if (x['type'] == 'enumerated' and x['id']== master_id and (x['relevanceCondition'] is not None and x['relevanceCondition'] != ""))]
